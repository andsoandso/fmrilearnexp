...
def exp(roi, table, verbose=True):
    ...
    for roipath, metapath in zip(roipaths, metapaths):
        ...
        # --
        # Create stats
        Xstat, fast_slow_stat = create_X_stats(
                X, 
                targets["trial_index"], 
                targets["fast_slow"])
        ...
        if verbose:
            print("Classifying...")
            print_label_counts(fast_slow_stat)
            print_clf_info(clf)

        truths, predictions = simpleCV(Xstat, fast_slow_stat, cv, clf, verbose)
        ...
        for truth, prediction in zip(truths, predictions):
            ...
            if verbose:
                ...
                print(classification_report(truth, prediction, 
                        target_names=sorted(np.unique(fast_slow_stat))))
                i += 1
...
if __name__ == "__main__":
    ...
    # --
    # Exp globals
    ACCURACY_TABLE_NAME = "fh_rt_stats_accuracy.txt"
