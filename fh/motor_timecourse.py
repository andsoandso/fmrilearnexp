...
usage: python ./motor_timecourse.py"""
...
from fmrilearn.analysis import fir, eva
from fmrilearn.preprocess.labels import (construct_targets, 
        construct_filter, filter_targets, create_y)
...
from wheelerdata.load.fh import get_motor_metadata_paths, get_roi_data_paths
...
def timecourses(roi, table):
    ...
    for roipath, metapath in zip(roipaths, metapaths):
        ...
        y =  create_y(targets["resps"])
        Xeva_i, yeva_i, timecourse_index_i = eva(X, targets["resps"], targets["trial_index"], MINWINDOW)
    ...
    df_eva['resps'] = yeva    ## Renaming for clarity
...
if __name__ == "__main__":
    ...
    for roi in ROIS:
        timecourses(roi, "fh_motor_eva_timecourse_{0}.csv".format(roi))
