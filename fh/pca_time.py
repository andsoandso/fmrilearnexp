...
usage: python ./pca_time.py name roifile cond [, filtfile]
"""
import sys, os
import pandas as pd
import numpy as np
...
for n, roi in enumerate(rois):
    ...
    for roipath, metapath in zip(roipaths, metapaths):
        ...
        for Xc, csname in zip(Xcs, csnames):
            # Look cond in csname
            cscond, csnum = csname.split("_")
            save_tcdf(
                    name=join_by_underscore(True, table, cscond), 
                    X=Xc, 
                    cond=cscond,
                    dataname=join_by_underscore(False, roiname,
                            spacetime.decompr.n_components, csnum),
                    index='auto',
                    header=header, 
                    mode=mode,
