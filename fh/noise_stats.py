...
-----
* Command line:
    python ./noise_stats.py   ## Runs many ROIs (runtime is ~10 hours)

* ipython:
    >>> from fmrilearnexp.fh.noise_stats import exp
    ...
    >>> exp("respXtime_rfx_mask",
            ...
            "fm_noise_stat_accuracy_test.txt"
            True)
"""
import os
import numpy as np
...
from fmrilearn.preprocess.labels import (construct_targets, 
        construct_filter, filter_targets, merge_labels)
...
def exp(roi, table, verbose=True):
    ...
    face/house data by noise response.
    ...
    for roipath, metapath in zip(roipaths, metapaths):
        ...
        # ----
        # Construct targets
        targets = construct_targets(noises=noises, 
        ...
        # --
        # Create stats
        Xstat, noise_level_stat = create_X_stats(
                X, 
                targets["trial_index"], 
                targets["noise_level"])
        ...
        if verbose:
            print("Classifying...")
            print_label_counts(noise_level_stat)
            print_clf_info(clf)

        truths, predictions = simpleCV(Xstat, noise_level_stat, cv, clf, verbose)
        ...
        for truth, prediction in zip(truths, predictions):
            ...
            if verbose:
                ...
                print(classification_report(truth, prediction, 
                        target_names=sorted(np.unique(noise_level_stat))))
                i += 1
...
if __name__ == "__main__":
    ...
    # --
    # Exp globals
    ACCURACY_TABLE_NAME = "fh_noise_stat_accuracy.txt"
